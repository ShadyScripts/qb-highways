...
local function SetWorkBlip(d)
    ...
    TriggerEvent('qb-highways:client:JobMarkers')
end
...
RegisterNetEvent('qb-highways:client:VehPick', function()
    local choice = math.random(1, #Config.JobVehicles)
    ElecVeh = Config.JobVehicles[choice]
    TriggerEvent('qb-highways:client:SpawnVehicle', ElecVeh)
end)
...
CreateThread(function()
    ...
    while true do
        ...
        -- if PlayerJob.name == "highways" then
                ...
                if #(pos - vector3(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z)) < 1.5 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                            ...
                            TriggerEvent('qb-highways:client:VehPick')
                        end
                    end  
                end
            end
...
RegisterNetEvent('qb-highways:client:SpawnVehicle', function(vehicleInfo)
...
RegisterNetEvent('qb-highways:client:JobMarkers', function(k, v)
    local inRange = false
    CompleteRepairs = 0
    while true do
        Wait(0)
                            ...
                            if IsControlJustReleased(0, 38) then
                                QBCore.Functions.Progressbar("repair_work", "Repairing Fault...", math.random(10000, 12000), false, true, {
                                    ...
                                    if CompleteRepairs <= 4 then
                                        QBCore.Functions.Notify("You've repaired the highways fault. Head to the next job.")
                                    else
                ...
                if #(pos - vector3(Config.Locations["payslip"].coords.x, Config.Locations["payslip"].coords.y, Config.Locations["payslip"].coords.z)) < 1.5 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if CompleteRepairs ~= 0 then
                            TriggerServerEvent('qb-highways:server:Payslip', CompleteRepairs)
                            CompleteRepairs = 0
                            for k,v in ipairs(JobsinSession) do
                                RemoveBlip(v.BlipId)
                            end
